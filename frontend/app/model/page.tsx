...
export default function Model() {
    ...
    const checkBackend = async () => {
      try {
        const response = await fetch(
          "https://deepfake-backend.deekshithreddy.site/health"
        );
        if (response.ok) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(
        "https://deepfake-backend.deekshithreddy.site/predict",
        {
          method: "POST",
          body: formData,
        }
  ...
  return (
      ...
      >
          ...
          <div className="w-full h-5/6 border border-neutral-200 shadow-sm rounded-lg">
            ...
            >
              ...
              {isDragActive ? (
                ...
                <div className="ml-2 font-sans md:text-lg text-sm font-medium">
                  {selectedFile.name}
                </div>
              ) : (
                <div>
        ...
        >
          ...
          <div className="w-full h-full flex flex-col border border-neutral-200 rounded-md">
            ...
            <ProgressBar
              title={"AI Generated"}
              progress={probabilities[2] * 100}
            />
            <div className="flex items-center space-x-1 ml-4 md:my-2 my-6">
              <ImageIcon size={17} />
              <div className="text-md font-sans font-semibold">Anomalies</div>
            </div>
            <div className="flex xl:space-x-10 space-x-2">
              <Image
                className="xl:h-38 xl:w-full h-32 w-full rounded-sm"
                height={50}
                width={100}
      ...
      <div>
        {backendStatus && (
          <div className="font-semibold font-sans text-lg">
            🟢 The Backend is up and running. Test out the model by uploading an
            image.
          </div>
        )}
        {!backendStatus && (
          <div className="flex flex-wrap font-sans text-lg font-semibold">
            🔴 The Backend is currently down. Ping me on
            <a
            ...
            >
              <span className="mr-1"></span> <Linkedin size={18} />
            </a>
    ...
    <div className="relative flex mt-2">
      <div className="group relative flex">
        ...
        <div className="absolute left-6 bottom-1 hidden group-hover:flex flex-col bg-gray-900 text-white text-sm p-2 rounded-lg shadow-lg w-72">
                  Note: The backend is self-hosted since the model is a bit too large for
                  the cloud charges to be inexpensive.
        </div>
      </div>
    </div>
    </div>
